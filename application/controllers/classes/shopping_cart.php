...
class shoppingCart extends base {
  ...
  function add_cart($products_id, $qty = '1', $attributes = '', $notify = true) {
    ...
    if ($notify == true) {
      $this->new_products_id_in_cart = $products_id;
    }
  ...
  function get_content_type($gv_only = 'false') {
    ...
    if ( $this->count_contents() > 0 ) {
      ...
      while (list($products_id, ) = each($this->contents)) {
        ...
        $free_ship_check = $db->query("select products_virtual, products_model, products_price, product_is_always_free_shipping from " 
                . TABLE_PRODUCTS . " where products_id = '" . zen_get_prid($products_id) . "'")->result();

        $virtual_check = false;
        if (preg_match('/^GIFT/', addslashes($free_ship_check[0]->products_model))) {
          $gift_voucher += ($free_ship_check[0]->products_price + $this->attributes_price($products_id)) * $this->contents[$products_id]['qty'];
        }
        ...
        if (isset($this->contents[$products_id]['attributes']) and $free_ship_check->fields['product_is_always_free_shipping'] != 2) {
          ...
          switch ($this->content_type) {
            ...
            break;
            case 'physical':
            if ($free_ship_check[0]->products_virtual == '1') {
              $this->content_type = 'mixed';
              if ($gv_only == 'true') {
                return $gift_voucher;
              } else {
