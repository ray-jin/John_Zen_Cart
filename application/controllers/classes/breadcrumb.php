...
//    echo 'breadcrumb ' . $i . ' of ' . $n . ': ' . $this->_trail[$i]['title'] . '<br />';
      ...
      if (isset($this->_trail[$i]['link']) && zen_not_null($this->_trail[$i]['link']) && !$skip_link ) {
        ...
        if ($this->_trail[$i]['title'] == HEADER_TITLE_CATALOG) {
          $trail_string .= ' <div itemscope itemtype="http://data-vocabulary.org/Breadcrumb"> <a href="' . HTTP_SERVER . DIR_WS_CATALOG . '" itemprop="url">' .'<span itemprop="title">'. $this->_trail[$i]['title'] . '</span></a></div>';
        } else {
          $trail_string .= ' <div itemscope itemtype="http://data-vocabulary.org/Breadcrumb"> <a href="' . $this->_trail[$i]['link'] . '" itemprop="url">' . '<span itemprop="title">' . $this->_trail[$i]['title'] . '</span></a></div>';
        }
      } else {
