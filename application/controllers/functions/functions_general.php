 ...
 */
  function charsetClean($string) {
	...
	function get_newsticker() {
		...
		$newsticker_query = "SELECT * FROM newsticker WHERE status = 1 order by dorder, id";		
		$newsticker = $db->Execute($newsticker_query);
		
		while (!$newsticker->EOF) {
			$newsticker_string .= '<li><a href="' . $newsticker->fields['url'] . '" target="_blank" title="' . $newsticker->fields['headline'] . '">' . (strlen($newsticker->fields['headline']) > 75 ? substr($newsticker->fields['headline'], 0, 75) . '...' : $newsticker->fields['headline']) . '</a></li>';
			$newsticker->MoveNext();
		}
